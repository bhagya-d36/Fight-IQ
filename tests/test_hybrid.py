...
def test_rrf_lifts_keyword_match_above_higher_cosine_nonmatch(make_store):
    store = _keyword_store(make_store)
...
def test_hybrid_gate_blocks_low_cosine_even_with_keyword_match(make_store):
    store = make_store(
...
def test_hybrid_falls_back_to_pure_cosine_ordering_when_no_keyword_match(make_store):
...
def test_hybrid_false_forces_pure_vector(make_store):
    store = _keyword_store(make_store)
...
def test_displayed_score_is_cosine_even_when_order_is_fused(make_store):
    store = _keyword_store(make_store)
    ...
    # Loose tolerance: Chroma's HNSW index stores embeddings as float32, so
    # the distance->similarity round-trip isn't bit-exact like the old
    # pure-Python cosine_similarity() was.
    assert abs(scores["b.md"] - 0.6) < 1e-6
