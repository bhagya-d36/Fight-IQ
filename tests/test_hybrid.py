...
def test_rrf_lifts_keyword_match_above_higher_cosine_nonmatch(fake_client):
    store = _keyword_store()
    client = fake_client([1.0, 0.0, 0.0])

    hits = rag.retrieve(store, client, "aspinall heavyweight champion", top_k=2, hybrid=True)

    assert [h["source"] for h in hits] == ["b.md", "a.md"]


def test_hybrid_gate_blocks_low_cosine_even_with_keyword_match(fake_client):
    store = {
        "dim": 3,
        "entries": [
    ...
    client = fake_client([1.0, 0.0, 0.0])  # cosine = 0.0, well below min_similarity

    hits = rag.retrieve(store, client, "aspinall heavyweight champion", top_k=3, min_similarity=0.35, hybrid=True)

    assert hits == []


def test_hybrid_falls_back_to_pure_cosine_ordering_when_no_keyword_match(fake_client):
    ...
    hits = rag.retrieve(store, client, "question", top_k=3, min_similarity=-1.0, hybrid=True)

    assert [h["source"] for h in hits] == ["a.md", "c.md", "b.md"]


def test_hybrid_false_forces_pure_vector(fake_client):
    store = _keyword_store()
    client = fake_client([1.0, 0.0, 0.0])

    hits = rag.retrieve(store, client, "aspinall heavyweight champion", top_k=2, hybrid=False)

    assert [h["source"] for h in hits] == ["a.md", "b.md"]


def test_displayed_score_is_cosine_even_when_order_is_fused(fake_client):
    store = _keyword_store()
    client = fake_client([1.0, 0.0, 0.0])
